...
RUN	mkdir /log && apt-get update -qq 1>>/log/apt-upd.log \
	...
	&& dpkg --add-architecture i386 && apt-get update -qq 1>>/log/apt-upd.log \
        && apt-get install -y -qq --no-install-recommends g++ g++-10 make libssl-dev wget unzip g++-mingw-w64-x86-64 mono-complete mono-vbnc nuget wine wine32 1>>/log/apt-inst.log \
        && cd /usr/local/src \ 
